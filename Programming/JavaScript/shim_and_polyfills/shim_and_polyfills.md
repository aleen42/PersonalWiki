...
For that problem, HTML5 has given out a more advanced API called **pushState**. The method `windows.history.pushState` and the event `windows.onpopstate` are both combined with this API to use handle navigation events asynchronously.
