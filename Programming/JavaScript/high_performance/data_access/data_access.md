...
Some optimizing JavaScript engines such as Safari's Nitro try to avid the traditional scope chain loopkup by indexing identifiers for faster resoultion. When a dynamic scope is involved, however, this optimization is no longer valid. For this reason, its recommended to use dynamic scopes only when absolutely necessary.