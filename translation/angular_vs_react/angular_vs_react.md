...
&nbsp; &nbsp; &nbsp; &nbsp;*需要注意的是：这是 React 模板中提供的一种最好的使用方式之一。在每个对象中，我们都为你生成了一个方法。因此你并不需要在 DOM 中存放任何的东西或进行任何的索引操作。而只需要把逻辑代码放在该方法里。如上面 DOM 元素的 onClick 事件处理中，我们提供了两种可用的选项：直接填写 lambda 表达式或如果使用 `{}` 记号的话，该记号会直接返回指向我们定义的函数的指针。*
