...
&nbsp; &nbsp; &nbsp; &nbsp;React 框架，使得我们能易于控制性能。当你实现了 shouldComponentUpdate 方法时，你就能选择一种你喜欢的比较方式 —— 模型层比较或展示层比较。当然，如果你的数据模型较为小型的话，你可以完全把比较的工作交给 React 的 vDom 比较器。可若是大型，或你要创建多个 DOM 元素的时候，你就可以通过自行实现该方法，来完成性能上的优化。因为在该方法里，你可以为脏检查（dirty-checking）定义属于自己的有效机制。
...
<p id="comment-6">[6]：<strong>异常堆栈跟踪</strong> 指的是计算机在某时刻关于当前内存堆栈的异常信息所作出一份跟踪报告。一般，通过该报告的描述，程序开发者能了解一个异常是如何在各种嵌套的程序中抛出，以快速定位出现异常的代码。</p>