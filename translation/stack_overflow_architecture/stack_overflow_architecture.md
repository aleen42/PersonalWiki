...
&#160; &#160; &#160; &#160;我们有一个配有Redis的L1/L2缓存系统。“L1"用于缓存网络服务器上的HTTP缓存，或者是任何一个正在运行的应用其缓存。”L2"则用于回落到Redis去取出所需要的值。我们的值是通过使用Marc Gravell发明的[protobuf-dot-net](https://github.com/mgravell/protobuf-net)组成[Protobuf格式](https://developers.google.com/protocol-buffers/)存储起来。对于客户端来说，我们会使用[StackExchange.Redis](https://github.com/StackExchange/StackExchange.Redis)——内部编写并。
