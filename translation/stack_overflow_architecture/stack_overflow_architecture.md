...
&#160; &#160; &#160; &#160;在这些网络服务器后是一层十分相像的“服务层”。它也是运行安装在Windows 2012R2上的IIS 8.5版本。该层会运行内部服务以支持网络层和其他内部系统的使用。其中，基于`http.sys`（不是IIS服务之后）运行着标签引擎的“Stack Server”和Providence API（基于IIS）分别扮演着重要角色。
