...
&#160; &#160; &#160; &#160;socket服务器本身是采用运行在网络层的原生socket。它在我们的开源库上是一个非常轻量的应用：[`StackExchange.NetGain`](https://github.com/StackExchange/NetGain)。在峰值期，我们会有大约50万的并发网络套接字连接打开。这代表着这里有非常多的浏览器。有趣的是，其实部分浏览器已经打开超过18个月的时间。对此，我们并不知道其中的缘由。或许，某人应该去检查一下这些开发者是否还存在。
