...
&#160; &#160; &#160; &#160;剧透：这里并没有许多能让你感到兴奋的东西。网络层正在使用 Elasticsearch 1.4 版本以下，并使用一个非常轻便且高性能的 `StackExchange.Elastic` 客户端去做非常优雅的搜索。不同于之前所探讨的是，我们并没有打算去简单地对这部分进行开源。因为，它仅仅揭露出我们正在使用的API其中的一个非常小的子集。而且，我们深信的是如果将其发行，对于开发者来说将会弊大于利。
