...
&#160; &#160; &#160; &#160;即将发行的版本就是恰恰在一条 **Release** 分支开始建立的时候获得一个通过分配的版本号 —— 不早不迟。此时，**develop** 分支就会显示该发行版本的修动部分。但是，它并不确定该发行版本最终是以0.3还是1.0发行，而直到该 **Release** 分支建立起来后才真正确定。版本号是在 **Release** 分支建立开始时所决定，并会由项目在版本号条目的规则所决定cai
