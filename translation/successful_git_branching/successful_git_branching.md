...
&#160; &#160; &#160; &#160;**Release** 分支用于为新产品的发行作准备。它只允许在最后的几分钟对产品进行一些修修补补的工作。更深一层来说就是，这些分支只允许微小漏洞的修复以及为一次发行准备元数据（版本号，构建日期等）。通过在一个 **Release** 分支上完成这些工作，**develop** 分支就可以获准接收为下一次大的发行所需要的新功能。
