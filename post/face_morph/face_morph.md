...
MARCH 11, 2016 BY SATYA MALLICK 32 COMMENTS
...
Bad Morphing Example ( Naive Blending ) 
...
(1)   \begin{align*}  x_m &= (1 - \alpha) x_i + \alpha x_j \\ y_m &= (1 - \alpha) y_i + \alpha y_j \end{align*}

Second, we need to find the intensity of the pixel at (x_m, y_m) using the following equation


(2)   \begin{equation*} M(x_m,y_m) = (1 - \alpha) I(x_i, y_i) + \alpha J(x_j, y_j) \end{equation*}
...
Point correspondences for Face Morphing
