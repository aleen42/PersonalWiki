## [轉載] Caching checklist [Back](./../post.md)
...
3. **Identify which resources can be cached by intermediaries(媒介)**: those with responses that are identical(完全相同) for all users are great candidates to be cached by a CDN and other intermediaries.
